...
public class AccessibilityOptionsScreenMixin {
    ...
    private static SimpleOption<?>[] serverWipe$getOptions(SimpleOption<?>[] original) {
        ...
        list.addAll(ClientCustomRegistries.OPTIONS
                .stream()
                .filter(x -> x.screen() == OptionScreen.ACCESSIBILITY)
                .map(Option::option)
                .toList()
        );

        list.removeIf(simpleOption ->
                simpleOption.text.getContent() instanceof TranslatableTextContent translatableTextContent &&
